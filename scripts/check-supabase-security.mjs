...
for (const file of files) {
  ...
  if (/auth\.role\(\)/i.test(content)) {
    fail(`${file}: replace auth.role() with TO authenticated / TO anon policy targets.`);
  }
  ...
  while ((match = viewRegex.exec(content)) !== null) {
    ...
    if (
      inPublicSchema &&
      !/with\s*\(\s*security_invoker\s*=\s*(?:true|'true'|on)\s*\)/i.test(statement)
    ) {
  ...
  for (const block of blocks) {
    ...
    const functionMatch = block.match(
      /create\s+or\s+replace\s+function\s+((?:[a-z0-9_]+\.)?[a-z0-9_]+)/i,
    );
    if (!functionMatch) {
      continue;
    }
