...
const reminders = [];
const touchesUi = changedPaths.some((path) => /^(app|components)\//.test(path));
...
const touchesGames = changedPaths.some((path) => /^packages\/games\//.test(path));
const touchesAgentTooling = changedPaths.some(
  (path) => /^(AGENTS\.md|\.agents\/|\.codex\/|plugins\/gamehub-integrations\/|scripts\/codex\/)/.test(path)
);
...
if (touchesDb) {
  reminders.push(
    "DB changes detected: confirm paired rollout/rollback files, schema parity, and migration validation."
  );
}

if (touchesGames) {
  reminders.push("Game package changes detected: confirm game integration tests pass and bundles are optimized.");
}
...
console.log("GameHub stop review reminders:");
