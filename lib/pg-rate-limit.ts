...
export async function checkPgRateLimit(identifier: string, maxRequests: number, windowSeconds: number, route = "default"): Promise<{ allowed: boolean; remaining: number; resetAt: number }> {
  try {
    ...
    const { data } = await (supabase as any).rpc("check_rate_limit", {
      p_identifier: identifier, p_route: route,
      p_max_requests: maxRequests, p_window_seconds: windowSeconds,
    }) as { data: { allowed: boolean; remaining: number; resetAt: number } | null; error: unknown };
