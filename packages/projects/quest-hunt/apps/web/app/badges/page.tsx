...
export default function BadgesPage() {
    ...
    const loadData = async () => {
      try {
        ...
        const badgesWithProgress = await Promise.all(
          allBadges.map((badge) => badgeService.getBadgeWithProgress(user.id, badge.id))
        );
  ...
  const badgesByCategory = filteredBadges.reduce<Record<string, BadgeWithProgress[]>>(
    (acc, badge) => {
      if (!acc[badge.type]) {
        acc[badge.type] = [];
      }
                        ...
                        <div key={category} className="space-y-4">
                          <div className="flex items-center gap-2">
                            ...
                            <Badge variant="outline" className="ml-2">
                              {categoryBadges.filter((b) => b.userProgress?.isUnlocked).length} /{' '}
                      ...
                      <div className="col-span-full text-center py-12 text-muted-foreground">
                        ...
                        <p className="text-sm mt-1">
                          {filter !== 'all'
                            ...
                            : 'Complete quests to unlock more badges'}
                        </p>
                      </div>
                    )}
                  </div>
            ...
            >
              ...
              <div>
                ...
                <div className="text-sm text-muted-foreground">
                  {stats?.byRarity[rarity as BadgeRarity]?.unlocked || 0} /{' '}
                  {stats?.byRarity[rarity as BadgeRarity]?.total || 0} unlocked
                </div>
              </div>
            </div>
          ))}
