...
export class InputManager {
  ...
  registerSequenceMacro(
    name: string,
    targetSequence: InputType[],
    onMatched?: (sequence: InputType[]) => void,
  ): () => void {
    const detector = new InputSequenceDetector(targetSequence, (sequence) => {
      // Emit a high-level macro event and call optional callback
      this.eventSystem.emit(`input:macro:${name}` as string, {
        name,
        sequence,
      });
      onMatched?.(sequence);
  ...
  private handlePointerUp = (e: MouseEvent): void => {
    ...
    if (timeSinceLastTap < this.doubleTapThreshold && distance < this.doubleTapDistanceThreshold) {
      // Double tap detected
