...
export type Scene = {
    ...
    body?: Record<Lang, string> | string;
    choices?: Choice[];
    render?: (props: {
        state: SaveState;
        go: (id: string) => void;
        setFlag: (key: string, val: any) => void;
    }) => React.ReactNode;
};
...
export type SaveState = {
    sceneId: string;
    ctx: EngineCtx;
    v?: number; // versioned for future migrations
};
...
export const guards = {
    ...
    hasItem: (id: string) => (ctx: EngineCtx) =>
        ctx.inventory?.includes(id) ?? false,
    varEquals: (key: string, value: any) => (ctx: EngineCtx) =>
        ctx.vars?.[key] === value,
};
