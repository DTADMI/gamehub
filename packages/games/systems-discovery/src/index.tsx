...
const BreathPuzzle: React.FC<{ onSolved: () => void }> = ({onSolved}) => {
    ...
    const rotate = (x: number, y: number) => {
        const nextRotation = ((state.grid[y * state.width + x].rotation + 90) % 360) as 0 | 90 | 180 | 270;
            ...
            return (
                <div>
                    ...
                    <ul className="list-disc ml-6">
                        <li>{t("sysdisc.wrap.b1")}: {String(flags["b1.route"])}</li>
                        <li>{t("sysdisc.wrap.b2")}: {String(flags["b2.plan"])}</li>
                        <li>{t("sysdisc.wrap.b3")}: {String(flags["b3.result"])}</li>
        ...
        render: ({state, setFlag, go}) => {
            ...
            return (
                <div>
                    ...
                    <div className="mb-2"><HomeostasisMeter value={Number(flags["bod.meter"] ?? 60)}/></div>
                    <ul className="list-disc ml-6 mb-3">
                        <li>Care Ally badge: {String(flags["bod.badges.careAlly"])}</li>
