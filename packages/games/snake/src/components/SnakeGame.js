...
import { CELL_SIZE, GAME_SPEED, GRID_SIZE, } from "../types/game";
...
export const SnakeGame = () => {
    ...
    const getRandomPosition = useCallback((exclude = []) => {
        let position;
        do {
            position = {
    ...
    const generateFood = useCallback((exclude) => {
        return getRandomPosition(exclude);
    }, [getRandomPosition]);
    // Generate obstacles
    const generateObstacles = useCallback((exclude) => {
        const obstacles = [];
    ...
    // Initialize game
    const initGame = useCallback((nextCfg) => {
    ...
    const restartWithConfig = useCallback((nextCfg) => {
        setConfig(nextCfg);
        initGame(nextCfg);
    }, [initGame]);
    // Check collision
    const checkCollision = (position, checkWalls = true) => {
        // Check walls
        if (checkWalls &&
            (position.x < 0 ||
        ...
        if (snake.some((segment, index) => index > 0 && segment.x === position.x && segment.y === position.y)) {
            return true;
        }
    ...
    useEffect(() => {
        ...
        const threshold = Math.min(SWIPE_THRESHOLD_MAX, Math.max(SWIPE_THRESHOLD_MIN, Math.floor(CELL_SIZE * 0.75)));
    ...
    useEffect(() => {
        ...
        const onClick = (e) => handleTap(e.clientX);
        const onTouchStart = (e) => {
            const t = e.touches[0];
            handleTap(t.clientX);
    ...
    useEffect(() => {
        ...
        if (config.hasPortals) {
            portals.forEach((portal) => {
                ...
                ctx.beginPath();
                ctx.arc(portal.entry.x * CELL_SIZE + CELL_SIZE / 2, portal.entry.y * CELL_SIZE + CELL_SIZE / 2, CELL_SIZE / 2, 0, Math.PI * 2);
                ctx.fill();
                // Exit portal (purple)
                ctx.fillStyle = "rgba(168, 85, 247, 0.7)";
                ctx.beginPath();
                ctx.arc(portal.exit.x * CELL_SIZE + CELL_SIZE / 2, portal.exit.y * CELL_SIZE + CELL_SIZE / 2, CELL_SIZE / 2, 0, Math.PI * 2);
                ctx.fill();
            });
        }
        // Draw food
        ...
        snake.forEach((segment, index) => {
            ...
            if (index === 0) {
                ...
                const gradient = ctx.createLinearGradient(segment.x * CELL_SIZE, segment.y * CELL_SIZE, (segment.x + 1) * CELL_SIZE, (segment.y + 1) * CELL_SIZE);
    ...
    useEffect(() => {
        ...
        window.addEventListener("snake:setDifficulty", handler);
        // Apply saved or default difficulty on mount
        try {
            const saved = localStorage.getItem("snakeDifficulty") || "normal";
            window.dispatchEvent(new CustomEvent("snake:setDifficulty", {
        ...
        return () => {
            window.removeEventListener("snake:setDifficulty", handler);
        };
    }, []);
    ...
    useEffect(() => {
        ...
        window.addEventListener("snake:restart", onRestartAction);
        window.addEventListener("snake:pauseToggle", onPauseToggleAction);
        return () => {
            window.removeEventListener("snake:restart", onRestartAction);
            window.removeEventListener("snake:pauseToggle", onPauseToggleAction);
        };
    }, [config, restartWithConfig]);
    // Handle game mode changes
    const buildConfigForMode = (base, mode) => {
    ...
    return (_jsx(GameContainer, { title: "Snake Game", description: `Eat the food to grow. Avoid walls and yourself! Score: ${score} | High Score: ${highScore}`, showParticleControls: false, children: _jsxs("div", { className: "p-4", children: [_jsxs("div", { className: "mb-3 flex flex-wrap items-center justify-center gap-2 sm:gap-3", children: [_jsx("span", { className: "text-sm text-gray-600 dark:text-gray-300", children: "Mobile Controls:" }), _jsxs("div", { className: "inline-flex rounded-md overflow-hidden border border-gray-300 dark:border-gray-700", children: [_jsx("button", { type: "button", onClick: () => setControlScheme("swipe"), className: `px-4 py-2 text-sm min-h-11 ${controlScheme === "swipe" ? "bg-blue-600 text-white" : "bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700"}`, children: "Swipe" }), _jsx("button", { type: "button", onClick: () => setControlScheme("joystick"), className: `px-4 py-2 text-sm min-h-11 border-l border-gray-300 dark:border-gray-700 ${controlScheme === "joystick" ? "bg-blue-600 text-white" : "bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700"}`, children: "Joystick" }), _jsx("button", { type: "button", onClick: () => setControlScheme("taps"), className: `px-4 py-2 text-sm min-h-11 border-l border-gray-300 dark:border-gray-700 ${controlScheme === "taps" ? "bg-blue-600 text-white" : "bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700"}`, children: "Taps" })] })] }), _jsx("div", { className: "mb-4 flex flex-wrap justify-center gap-2", children: ["classic", "obstacles", "portal", "speed"].map((mode) => (_jsx("button", { onClick: () => handleModeChange(mode), className: `px-4 py-2 rounded-md min-h-11 ${config.mode === mode
                            ? "bg-blue-600 text-white"
                            : "bg-gray-200 hover:bg-gray-300 dark:bg-gray-700 dark:hover:bg-gray-600"}`, children: mode.charAt(0).toUpperCase() + mode.slice(1) }, mode))) }), pendingMode && (_jsxs("div", { className: "mx-auto mb-4 max-w-md rounded-md border bg-card text-card-foreground p-3 shadow-sm", children: [_jsxs("p", { className: "text-sm mb-2", children: ["Restart the game in ", _jsx("b", { children: pendingMode }), " mode? Current progress will be lost."] }), _jsxs("div", { className: "flex gap-2 justify-end", children: [_jsx("button", { className: "px-4 py-2 text-sm rounded-md min-h-11 bg-secondary text-secondary-foreground hover:bg-secondary/80", onClick: () => {
                                        setPendingMode(null);
                                        setIsPaused(false);
                                    }, children: "Cancel" }), _jsx("button", { className: "px-4 py-2 text-sm rounded-md min-h-11 bg-primary text-primary-foreground hover:bg-primary/90", onClick: () => {
                                        const nextCfg = buildConfigForMode(config, pendingMode);
                                        setPendingMode(null);
                                        restartWithConfig(nextCfg);
                                    }, children: "Restart" })] })] })), _jsx("div", { className: "flex justify-center", children: _jsx("canvas", { ref: canvasRef, width: config.gridSize * CELL_SIZE, height: config.gridSize * CELL_SIZE, "data-testid": "snake-canvas", className: "bg-white rounded-lg shadow-md block", "aria-label": "Snake playfield", tabIndex: 0, onClick: () => {
                            if (!gameStarted) {
                                setGameStarted(true);
                            }
                            else if (isPaused) {
                        ...
                        } }) }), (!gameStarted || isPaused) && !gameOver && (_jsx("button", { "aria-label": !gameStarted ? "Tap to start" : "Tap to resume", className: "mt-3 w-full max-w-xl mx-auto flex items-center justify-center bg-black/40 text-white text-base sm:text-lg font-semibold select-none rounded-md h-16", onClick: () => {
                        if (!gameStarted) {
                            setGameStarted(true);
                        }
                        else {
                    ...
                    }, children: !gameStarted ? "Tap to start" : "Paused — Tap to resume" })), controlScheme === "joystick" && (_jsx("div", { className: "mt-4 flex justify-center", children: _jsx(VirtualJoystick, { onDirection: (dir) => {
                        ...
                        } }) })), _jsx("div", { className: "mt-4 text-center text-base sm:text-sm text-gray-600 dark:text-gray-400", children: _jsxs("p", { children: ["Use arrow keys to move | Space to ", isPaused ? "resume" : "pause"] }) }), _jsxs("div", { className: "mt-4 flex justify-between text-base sm:text-sm", children: [_jsxs("div", { children: ["Score: ", _jsx("span", { className: "font-bold", children: score })] }), _jsxs("div", { children: ["High Score: ", _jsx("span", { className: "font-bold", children: highScore })] }), _jsxs("div", { children: ["Speed:", " ", _jsx("span", { className: "font-bold", children: config.mode === "speed" ? "Fast" : "Normal" })] })] })] }) }));
};
const VirtualJoystick = ({ onDirection }) => {
    const padRef = useRef(null);
    const [active, setActive] = useState(false);
    const knobRef = useRef(null);
    // Convert movement vector to a cardinal direction
    const vectorToDir = useCallback((dx, dy) => {
    ...
    const handleMoveFromEvent = useCallback((clientX, clientY) => {
    ...
    return (_jsx("div", { className: "select-none", children: _jsx("div", { ref: padRef, "aria-label": "Virtual joystick", className: "relative h-32 w-32 rounded-full bg-gray-200 dark:bg-gray-700 shadow-inner", style: { touchAction: "none" }, children: _jsx("div", { ref: knobRef, className: `absolute left-1/2 top-1/2 h-16 w-16 -translate-x-1/2 -translate-y-1/2 rounded-full shadow-md ${active ? "bg-blue-500" : "bg-white dark:bg-gray-400"}` }) }) }));
};
