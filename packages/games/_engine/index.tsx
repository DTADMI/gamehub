...
export { useCanvasSize, useGameLoop, useKeyboardInput, Vec2 } from "./game-loop";
  ...
  return (
    <div className="mx-auto max-w-3xl p-4">
      ...
      <div className="mb-3" aria-label="Inventory" role="region">
        <div className="mb-1 text-sm font-medium">Inventory ({state.inventory.length})</div>
          ...
          {state.inventory.length === 0 ? (
            ...
            state.inventory.map((item) => (
              <button
                ...
                onClick={() => {
                  /* reserved for future inspect/use */
                }}
              >
                {item}
              </button>
      ...
      <div
        role="status"
        aria-live="polite"
        className="mb-3 min-h-[1.5rem] text-sm opacity-80"
...
export function HotspotButton({
  ...
  return (
    <button
      data-testid={testId}
      onClick={onClick}
      className="bg-primary text-primary-foreground min-h-[44px] rounded px-4 py-2 hover:opacity-90 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2"
    >
      {children}
    </button>
  );
...
export function ChoiceList({
  ...
  choices: Choice[];
  onPick: (id: string, next: SceneId) => void;
}) {
  return (
