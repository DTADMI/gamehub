    ...
    () => ({
      ...
      KEYPAD_ZOOM: {
        ...
        choices: [
          {
            id: "exit",
            text: { en: t("rod.keypad.back"), fr: t("rod.keypad.back") },
            target: "HALLWAY",
          },
        ],
      },
      WIRES_PANEL: {
        ...
        choices: [
          {
            id: "exit",
            text: { en: t("rod.wires.back"), fr: t("rod.wires.back") },
            target: "HALLWAY",
          },
        ],
      },
      ...
      <div className="mx-auto max-w-2xl p-4">
        ...
        {sceneId === "HALLWAY" && (
          <div className="mb-6 grid grid-cols-2 gap-4">
            <button
              className="hover:bg-muted rounded-lg border p-8 text-center"
        ...
        {sceneId === "KEYPAD_ZOOM" && (
          <div className="bg-muted mb-6 rounded-lg p-6">
            <div className="mx-auto mb-4 grid max-w-[200px] grid-cols-3 gap-2">
              {[1, 2, 3, 4, 5, 6, 7, 8, 9, 0].map((n) => (
                <button
                  key={n}
                  className="bg-background hover:bg-accent h-12 min-h-11 w-12 rounded border"
              ...
              <button
                className="bg-background hover:bg-accent col-span-2 h-12 rounded border"
                onClick={() => setKeypad((s) => clearKeypad(s))}
              >
                {t("rod.keypad.clear")}
              </button>
              <button
                className="bg-primary text-primary-foreground h-12 rounded border"
                onClick={() => {
            ...
            <div className="h-8 text-center font-mono text-xl tracking-widest">
        ...
        {sceneId === "WIRES_PANEL" && (
          <div className="bg-muted mb-6 rounded-lg p-6">
            <div className="flex flex-col gap-4">
              {["red", "blue"].map((color) => (
                <div key={color} className="flex items-center gap-4">
                  ...
                  <select
                    className="min-h-11 rounded border p-1"
                    onChange={(e) => {
                  ...
                  >
                    <option value="">
                      {lang === "fr" ? "Sélectionner..." : "Select connection..."}
                    </option>
            ...
            {ctx.flags["door.wiresSolved"] && (
              <p className="mt-4 text-center font-bold text-green-500">{t("rod.wires.solved")}</p>
            )}
          </div>
        )}
        ...
        {sceneId === "HALLWAY" &&
          ...
          ctx.flags["door.wiresSolved"] && (
            <button
              className="bg-primary text-primary-foreground mb-6 w-full animate-pulse rounded-lg p-4"
