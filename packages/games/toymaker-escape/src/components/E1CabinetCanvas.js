 ...
 */
export function E1CabinetCanvas({ width = 480, height = 160, onLatchReveal, }) {
    const canvasRef = React.useRef(null);
    const engineRef = React.useRef(null);
    const macroUnsubRef = React.useRef(undefined);
    const unsubEventRef = React.useRef(undefined);
    ...
    React.useEffect(() => {
        ...
        macroUnsubRef.current = engine.input.registerSequenceMacro("holdThenDrag", ["pointerdown", "longpress", "swipe"]);
    ...
    return (_jsx("div", { className: "inline-block", children: _jsx("canvas", { ref: canvasRef, width: width, height: height, className: "rounded border border-dashed border-muted-foreground/50", "aria-label": "Cabinet panel canvas" }) }));
}
