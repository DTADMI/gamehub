                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-2 mb-2">
                                <button className="px-3 py-2 rounded bg-muted"
                                        onClick={() => {
                                            setPipes(s => {
                                                const nextRotation = (((s.grid[0]?.rotation || 0) + 90) % 360) as 0 | 90 | 180 | 270;
                                                return evaluatePipes(setTileRotation(s, 0, 0, nextRotation));
                                            });
                                        }}>{t('tme.e1.panel.rotate00')}</button>
                                <button className="px-3 py-2 rounded bg-muted"
                                        onClick={() => {
                                            setPipes(s => {
                                                const nextRotation = (((s.grid[2]?.rotation || 0) + 90) % 360) as 0 | 90 | 180 | 270;
                                                return evaluatePipes(setTileRotation(s, 2, 0, nextRotation));
                                            });
                                        }}>{t('tme.e1.panel.rotate20')}</button>
                                <button className="px-3 py-2 rounded bg-muted"
