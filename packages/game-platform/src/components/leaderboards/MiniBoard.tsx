...
function Row({ row, highlight }: { row: LeaderboardEntry; highlight: boolean }) {
  return (
    <div
      className={`flex items-center justify-between py-1.5 text-sm ${highlight ? "rounded-md bg-blue-50 px-2 dark:bg-blue-900/20" : ""}`}
    >
      <div className="text-muted-foreground w-8 shrink-0">{row.rank}</div>
...
export default function MiniBoard({ gameType, limit = 10, className }: MiniBoardProps) {
  ...
  if (!user) {
    return (
      <div className="text-muted-foreground mt-4 rounded-lg border border-dashed border-gray-300 p-4 text-center text-sm dark:border-gray-700">
  ...
  return (
    <div
      className={`mt-4 rounded-lg border border-gray-200 bg-white/70 p-4 dark:border-gray-800 dark:bg-gray-900/60 ${className ?? ""}`}
    >
