...
import { GameContainer, soundManager } from "@/lib/game-platform";
...
export const MemoryGame: React.FC = () => {
  ...
  useEffect(() => {
    if (cards.length > 0 && cards.every((card) => card.isMatched)) {
      ...
      window.dispatchEvent(
        new CustomEvent("memory:gameover", {
          detail: { score: moves },
        }),
      );
    }
  ...
  useEffect(() => {
    ...
    const unmatched = cards.map((c, i) => (c.isMatched ? -1 : i)).filter((i) => i >= 0);
  ...
  }, [autoCompleteLastPair, cards, flippedIndices.length, gameOver, isProcessing, playSound]);
  ...
  const pairsInPlay = useMemo(
    () => cards.length / 2 || (difficulty === "easy" ? 6 : difficulty === "medium" ? 8 : 12),
    ...
    >
      <div className="overflow-hidden p-4">
        ...
        <div className="mb-6 flex flex-col items-center justify-center gap-4 text-center sm:flex-row">
          <div>
            <label className="mr-2 text-gray-700 dark:text-gray-300">Theme:</label>
            <select
              value={theme}
              onChange={(e) => setTheme(e.target.value as ThemeKey)}
              className="focus:border-primary focus:ring-primary rounded-md border border-gray-300 bg-white px-2 py-1 text-sm text-gray-900 focus:ring-1 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100"
          ...
          {cards.map((card, index) =>
            removedIds.has(card.id) ? (
              ...
              <div
                ...
                className="pointer-events-none aspect-square rounded-xl opacity-0"
                aria-hidden
                role="presentation"
              />
            ) : (
              ...
              >
                {/* Back */}
                <div className="absolute inset-0 flex items-center justify-center rounded-xl bg-gradient-to-br from-blue-600 to-indigo-600 text-2xl font-semibold text-white [backface-visibility:hidden]">
                  ✨
                </div>
                {/* Front */}
                <div className="absolute inset-0 flex [transform:rotateY(180deg)] items-center justify-center rounded-xl bg-white text-4xl [backface-visibility:hidden] dark:bg-gray-800">
