...
import { GameContainer, ParticlePool, soundManager, useGameSettings, submitScore } from "@/lib/game-platform";
...
export default function BreakoutGame() {
  ...
  const [mouseControl, setMouseControl] = useState<boolean>(
    () => getBreakoutSettings().mouseControl,
  );
      ...
      if (started && !paused && !over) {
        ...
        if (sticky && sticky.captured) {
          ...
          if (
            ...
            const angleBase =
              clamp(sticky.offset / (statePaddle.width / 2), -1, 1) * MAX_BOUNCE_ANGLE;
        ...
        if (Math.abs(ndx) < MIN_HORIZ_COMPONENT * 0.5) {
          ...
          const hy = -Math.sign(ndy || -1) * Math.sqrt(Math.max(0, desired * desired - hx * hx));
          ndx = hx;
          ndy = hy;
        }
        ...
        const remaining = nextBricks.reduce(
          (acc, col) => acc + (col?.filter((b) => b && b.health > 0).length || 0),
          0,
        );
          ...
          for (const p of stateFalling) {
            ...
            if (caught) {
              ...
              if (p.type === "extraLife") {
                ...
                const dur =
                  p.type === "laser" || p.type === "thru" || p.type === "fireball"
  ...
  const { enableParticles, particleEffect, mode, isAuthenticated, isSubscriber } =
    useGameSettings();
  ...
  useEffect(() => {
    ...
    enableParticlesRef.current =
      enableParticles === undefined ? enableParticlesRef.current : !!enableParticles;
  ...
  return (
    ...
    >
      ...
      <div className="mb-3">
        <div className="mx-auto flex min-h-[44px] max-w-[960px] flex-wrap items-center justify-between gap-2 rounded-md bg-gray-100 px-3 py-2 whitespace-nowrap shadow-sm dark:bg-gray-800/80">
          <div className="flex items-baseline gap-1.5">
            <div className="text-[10px] tracking-wide text-gray-500 uppercase dark:text-gray-400">
              Score
            </div>
            <div className="text-lg font-bold tabular-nums md:text-xl">{score}</div>
          </div>
          <div className="hidden items-baseline gap-1.5 sm:flex">
            <div className="text-[10px] tracking-wide text-gray-500 uppercase dark:text-gray-400">
              High
            </div>
            <div className="text-base font-semibold tabular-nums md:text-lg">{highScore}</div>
          </div>
          <div className="flex items-center gap-1.5">
            <div className="text-[10px] tracking-wide text-gray-500 uppercase dark:text-gray-400">
              Lives
            </div>
            <div className="text-lg md:text-xl" aria-live="polite" aria-label={`Lives ${lives}`}>
              {"❤️".repeat(lives)}
            </div>
          </div>
          ...
          <div className="flex items-baseline gap-1.5">
            ...
            <div className="text-base font-semibold md:text-lg">{level}</div>
          </div>
          {activeRef.current && (
            <div className="flex items-center gap-1.5">
              ...
              {(() => {
                ...
                return (
                  <span
                    className={`inline-flex items-center rounded ${colorClass[t]} px-1.5 py-0.5 text-[11px] text-white md:text-xs`}
                    style={{
                      maxWidth: 180,
                      overflow: "hidden",
        ...
        <div className="relative">
          ...
          {(!gameStarted || isPaused) && !gameOver && (
            <button
              ...
              className="absolute inset-0 flex items-center justify-center bg-black/40 text-base font-semibold text-white select-none sm:text-lg"
              onClick={() => {
                if (!gameStarted) {
                  setGameStarted(true);
                  soundManager.playMusic("background");
          ...
          {gameOver && (
            <div className="absolute inset-0 flex items-center justify-center bg-black/50 text-white">
              <div className="space-y-3 p-4 text-center">
                ...
                <button
                  className="mt-2 rounded-md bg-blue-600 px-4 py-2 hover:bg-blue-700"
                  onClick={startGame}
                >
                  Restart
                </button>
      ...
      <div className="mt-3 text-center">
        {!gameStarted && !gameOver && !awaitingNext && (
          <button
            onClick={startGame}
            className="rounded-md bg-blue-600 px-5 py-2 text-white transition-colors hover:bg-blue-700"
          >
            Start Game
          </button>
        )}
        {(gameStarted || awaitingNext) && (
          <button
            ...
            className={`rounded-md px-5 py-2 text-white transition-colors ${awaitingNext ? "bg-emerald-600 hover:bg-emerald-700" : "bg-gray-600 hover:bg-gray-700"}`}
        ...
        {gameStarted && !awaitingNext && !isPaused && (
          <button
            ...
            disabled={boosters <= 0}
            className={`ml-2 rounded-md px-5 py-2 transition-colors ${boosters > 0 ? "bg-amber-600 text-white hover:bg-amber-700" : "cursor-not-allowed bg-gray-400 text-gray-100"}`}
        ...
        <div className="mx-auto mt-2 flex w-full max-w-[960px] items-center justify-center gap-4">
        ...
        <details className="mx-auto mt-2 w-full max-w-[960px] text-left opacity-70">
      ...
      <div className="mt-6">
        {/* Mobile */}
        <details className="rounded-lg border border-gray-200 sm:hidden dark:border-gray-700">
          <summary className="cursor-pointer px-4 py-2 text-base font-semibold select-none">
            How to Play
          </summary>
          <ul className="space-y-1 px-4 pt-1 pb-3 text-sm text-gray-600 dark:text-gray-300">
      ...
      <div className="mt-4">
        ...
        <details className="rounded-lg border border-gray-200 sm:hidden dark:border-gray-700">
          ...
          <div className="grid grid-cols-1 gap-3 px-4 pt-1 pb-3 text-sm">
            <PowerUpCardMobile
              title="Fast"
        ...
        {/* Desktop/tablet */}
        <div className="hidden sm:block">
          <h3 className="mb-2 text-lg font-semibold">Power-Ups</h3>
