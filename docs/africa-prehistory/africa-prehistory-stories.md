# Africa Prehistory - Paths of Many (Stories & Scenarios Tracker)

Purpose

- Track scenarios, ensure regional diversity and community focus, and guide expert review.

Related docs

- Design Doc: docs/africa-prehistory/africa-prehistory-design.md
- Designer Brief (EN+FR): docs/africa-prehistory/africa-prehistory-designer-brief.md

Status key

- Draft, Review, Ready for Art, In Art, Ready for Eng, In Dev, QA, Shipped

Packs overview (3 scenes + wrap)

- Sahara & Sahel Pathways (SS)
- Forests & Rivers - Congo/Niger (FR)
- Rift & Horn (RH)
- Great Lakes & Highlands (GL)
- Coasts & Crossings - Atlantic/Indian (CC)

Index
| ID | Title | Pack | Status | Owner |
|---|---|---|---|---|
| SS1 | Oasis Caravan Circle | SS | Draft | Narrative |
| SS2 | Copper to Iron | SS | Draft | Narrative |
| SS3 | Wayfinding Across Dunes | SS | Draft | Narrative |
| SS-W | Wrap - Sahara & Sahel | SS | Draft | Narrative |
| FR1 | River Canoe Post | FR | Draft | Narrative |
| FR2 | Forest Knowledge | FR | Draft | Narrative |
| FR3 | Mancala & Math | FR | Draft | Narrative |
| FR-W | Wrap - Forests & Rivers | FR | Draft | Narrative |
| RH1 | Highland Terraces | RH | Draft | Narrative |
| RH2 | Port Monsoons | RH | Draft | Narrative |
| RH3 | Scripts & Scrolls | RH | Draft | Narrative |
| RH-W | Wrap - Rift & Horn | RH | Draft | Narrative |
| GL1 | Cattle & Commons | GL | Draft | Narrative |
| GL2 | Iron & Gardens | GL | Draft | Narrative |
| GL3 | Courtyard Justice | GL | Draft | Narrative |
| GL-W | Wrap - Great Lakes & Highlands | GL | Draft | Narrative |
| CC1 | Currents & Canoes | CC | Draft | Narrative |
| CC2 | Dhows & Markets | CC | Draft | Narrative |
| CC3 | Tides & Safety | CC | Draft | Narrative |
| CC-W | Wrap - Coasts & Crossings | CC | Draft | Narrative |

Story template

- Goal
- Age framing
- Hook
- Core beats (3–6)
- Puzzles/Interactions
- Choices/Flags
- Kindness/Accessibility guardrails
- Assets
- Copy notes
- Open questions
- Fact‑check notes
- Review checklist

Detailed entries

SS1 Oasis Caravan Circle (Sahara & Sahel)

- Goal: Show shared rules for water, rest, and safety at a desert oasis.
- Age: 9–12 base; 15+ toggles add climate and trade context.
- Hook: Caravan bells and date palms; all circles gather to plan water turns.
- Beats: check cistern; assign turns; hear news; prepare night watch.
- Puzzles: bucket‑chain logic; ledger fairness - balance cups across groups.
- Choices/Flags: `ss1.splitPolicy = equal|need|alternate`; `ss1.nightWatch = rota|volunteer`.
- Guardrails: no graphic hardship; respectful languages; context captions.
- Assets: `bg_ss_1.avif`, `prop_ss_1_oasis_ledger.svg`, `ui_ss_meter.svg`.
- Copy notes: emphasize fairness emerges from group rules.
- Fact‑check: oasis cistern practices; caravan routines.
- Review: assets listed; captions drafted; contrast okay.

SS2 Copper to Iron

- Goal: Collective learning in metallurgy via itinerant smiths and apprentices.
- Hook: Furnace base laid; many hands prepare bellows and ore.
- Beats: gather ore/charcoal; build furnace; temper tools; share know‑how.
- Puzzles: furnace assembly; temperature balance minigame.
- Flags: `ss2.method = bloomery|shaft`; `ss2.tempering = water|oil`.
- Guardrails: abstracted furnace visuals; safety captions.
- Assets: `prop_ss_2_furnace_parts.svg`.

SS3 Wayfinding Across Dunes

- Goal: Teams plan routes using stars, winds, and dune shapes.
- Puzzles: star chart alignment; dawn/dusk march planner with heat constraints.
- Flags: `ss3.route = north|ridge|wadi`.

SS-W Wrap - Sahara & Sahel

- Goal: Reflection on rules and shared labor; award `collect_ss_badge.svg`.

FR1 River Canoe Post (Congo/Niger)

- Goal: Exchanges of salt, fish, beads, and news.
- Puzzles: balance bundles; river current matching.
- Flags: `fr1.bundle = fair|greedy`; `fr1.route`.

FR2 Forest Knowledge

- Goal: Plant lore and safe paths.
- Puzzles: identify edible/medicinal icons; path memory.
- Guardrails: avoid “exoticizing”; schematic icons.

FR3 Mancala & Math

- Goal: Strategy game encodes counting and planning.
- Puzzle: mancala capture logic; planning rounds.

FR-W Wrap - Forests & Rivers

- Badge: `collect_fr_badge.svg`.

RH1 Highland Terraces (Rift & Horn)

- Goal: Soil and water care.
- Puzzles: terrace module placement; erosion balance.

RH2 Port Monsoons

- Goal: Seasonal calendars for safe sailing.
- Puzzle: monsoon wheel; harbor duties schedule.

RH3 Scripts & Scrolls

- Goal: Scripts as community knowledge webs.
- Puzzle: script tile matching; scribe rota.

RH-W Wrap - Rift & Horn

- Badge: `collect_rh_badge.svg`.

GL1 Cattle & Commons (Great Lakes)

- Goal: Pastoral rules and shared grazing.
- Puzzle: commons tokens ledger; drought plan.

GL2 Iron & Gardens

- Goal: Toolmaking and enset/banana gardens.
- Puzzle: tool assembly; garden layout.

GL3 Courtyard Justice

- Goal: Mediation and social safety.
- Puzzle: hearing order; restitution plan.

GL-W Wrap - Great Lakes

- Badge: `collect_gl_badge.svg`.

CC1 Currents & Canoes (Coasts)

- Goal: Navigation and outriggers.
- Puzzle: currents map; outrigger balance.

CC2 Dhows & Markets

- Goal: Indian Ocean hubs; cultural blends.
- Puzzle: goods vs. ballast; market tokens.

CC3 Tides & Safety

- Goal: Coastal watch and fire codes.
- Puzzle: tide clock; watch rota.

CC-W Wrap - Coasts & Crossings

- Badge: `collect_cc_badge.svg`.

Universal Review Checklist

- Beats and puzzles clear; flags defined; accessibility assets listed; sensitivity captions added; sources linked.

Changelog

- v1: Initial packs and detailed entries added.
