...
GameHub is a monolithic application built with Next.js 16, a TypeScript/NestJS backend, and multiple integrated games
and projects. This repository follows a monorepo structure using `pnpm` workspaces and `Turborepo`.

### Monorepo Structure

- `apps/app`: Next.js 16 application (Main Entry Point).
- `apps/api`: NestJS/Prisma backend (Single Source of Truth for Database and Logic).
- `packages/shared`: Shared logic, metadata (games/projects), engine, and UI components.
- `packages/games`: Modular game implementations (Breakout, Snake, Memory, etc.).
- `packages/projects`: Integrated full-stack applications (StoryForge, QuestHunt, etc.).

---

### Command Reference

The project uses **Turborepo** for efficient task execution. You can run commands at the root for the entire monorepo or
filter for specific packages.

#### 1. Root Level (Whole Monorepo)

| Task        | Command         | Description                                |
|:------------|:----------------|:-------------------------------------------|
| **Install** | `pnpm install`  | Install all dependencies                   |
| **Dev**     | `pnpm dev`      | Run all apps (App & API) in dev mode  |
| **Build**   | `pnpm build`    | Build all apps and packages                |
| **Lint**    | `pnpm lint`     | Lint the entire codebase                   |
| **Test**    | `pnpm test`     | Run all unit and integration tests         |
| **E2E**     | `pnpm test:e2e` | Run Playwright end-to-end tests (App) |

#### 2. Individual Apps & Packages

Run commands for a specific part of the monolith using `--filter`.

| Target       | Dev Command                      | Build Command                       | Lint Command                       |
|:-------------|:---------------------------------|:------------------------------------|:-----------------------------------|
| **App** | `pnpm --filter gamehub dev`      | `pnpm --filter gamehub build`       | `pnpm --filter gamehub lint`       |
| **API**      | `pnpm --filter @gamehub/api dev` | `pnpm --filter @gamehub/api build`  | `pnpm --filter @gamehub/api lint`  |
| **Shared**   | -                                | `pnpm --filter @games/shared build` | `pnpm --filter @games/shared lint` |
| **Snake**    | -                                | `pnpm --filter @games/snake build`  | `pnpm --filter @games/snake lint`  |
| **Memory**   | -                                | `pnpm --filter @games/memory build` | `pnpm --filter @games/memory lint` |

*Note: Replace `@games/snake` with any other package name from `packages/games` or `packages/projects`.*

### Centralized Infrastructure & CI/CD

- **Docker**: The application uses a single centralized `Dockerfile` at the root for both API and App deployment.
  See [docs/docker-centralization.md](docs/docker-centralization.md).
- **CI/CD**: GitHub Actions workflows in `.github/workflows` manage building, testing, and deployment for the entire
  monolith.
- **E2E Testing**: Playwright tests are managed from the root `playwright.config.ts` and target the whole application.

---

### Centralized Backend & Database

The project maintains **only one database** managed by Prisma in `apps/api`. All projects and games that require
persistent storage must integrate with the unified schema in `apps/api/prisma/schema.prisma`.

To update the database:

1. Modify `apps/api/prisma/schema.prisma`.
2. Run `pnpm --filter @gamehub/api prisma generate`.
3. Run `pnpm --filter @gamehub/api prisma migrate dev`.
...
- StoryForge
  - Design: docs/story-forge/story-forge-design.md
- QuestHunt
  - Design: docs/quest-hunt/quest-hunt-design.md
- LibraKeeper
  - Design: docs/libra-keeper/libra-keeper-design.md
...
App route: `/signin` — a single page with tabs (Sign In/Sign Up), Google/GitHub buttons, “Forgot password” link,
